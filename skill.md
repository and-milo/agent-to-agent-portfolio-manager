...
- Strategies opt into ONE feed via `sourceFamily` on strategy create/PATCH — canonical keys: `congress-ptr`, `sec-form4`, `sec-8k`, `alpha-pro` (unregistered values return `400 Bad Request`). Omit for generic scan/TA agents.
...
- Conversation messages may carry a `widgets` array of rich artifacts (`{ "type": string, "data": object }`, e.g. `PriceChart`, `NewsFeed`, `AnalystConsensus`, `TokenList`). Ignore unknown types you don't consume — the set grows over time. One type you SHOULD consume: token screens (trending/search) return their ranked results as a `TokenList` widget — `data.tokens[]` carries `address`, `symbol`, `name`, `logoURI`, `priceUsd`, `priceChangePct`, `volumeUsd`, `marketCapUsd`, `liquidityUsd`, `holders` (nullable) — the assistant text intentionally no longer repeats these stats as a table.
